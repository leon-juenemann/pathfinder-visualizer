...
class GridController {
  ...
  bindTileHandlers() {
    ...
    for (let x = 0; x < width; x++) {
      for (let y = 0; y < height; y++) {
        this.gridView.bindTileMouseDown(x, y, this.handleTileMouseDown);
        this.gridView.bindTileClick(x, y, this.handleTileClick);
        this.gridView.bindTileMouseOver(x, y, this.handleTileMouseOver);
        this.gridView.bindTileMouseOut(x, y, this.handleTileMouseOut);
      }
    }
  }
  handleMouseUp = () => {
  ...
  handleTileMouseDown = (x, y) => {
    ...
    if (!this.movingStartTile && !this.movingEndTile) {
      ...
      if (!this.gridModel.isStart(x, y) && !this.gridModel.isEnd(x, y)) {
